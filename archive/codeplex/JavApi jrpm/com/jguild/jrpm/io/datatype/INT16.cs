...
namespace com.jguild.jrpm.io.datatype
{
     ...
     */
    public sealed class INT16 : DataTypeIf
    {
        private static readonly java.util.logging.Logger logger = RPMFile.logger;
        private const int DATA_WIDTH = 2;
        private short[] data;
        private long size;

        internal INT16(short[] data)
        {
            this.data = data;
         ...
         */
        public static INT16 readFromStream(java.io.DataInputStream inputStream,
                                           IndexEntry indexEntry)
        {//throws IOException {
            if (indexEntry.getType() != RPMIndexType.INT16)
            {
                throw new java.lang.IllegalArgumentException("Type <" + indexEntry.getType()
                        + "> does not match <" + RPMIndexType.INT16 + ">");
            }

            short[] data = new short[(int)indexEntry.getCount()];

            for (int pos = 0; pos < indexEntry.getCount(); pos++)
            {
                data[pos] = inputStream.readShort();
            }

            INT16 int16Object = new INT16(data);

            if (logger.isLoggable(java.util.logging.Level.FINER))
            {
                logger.finer(int16Object.toString());
            }

            // int16Object.size = indexEntry.getType().getSize()
            // * indexEntry.getCount();
        ...
        {
            return new java.lang.Short(this.data[i]);
        }
        ...
        public override String ToString()
        {
            ...
            {
                buf.append(this.data[pos] & 0x0FFFF);
