...
namespace biz.ritter.javapi.util
{
     ...
     */
        ...
        public Object clone() {
            try {
                BitSet clone = (BitSet) MemberwiseClone();
        ...
        private void growLength(int len) {
            long[] tempBits = new long[java.lang.Math.max(len, bits.Length * 2)];
            java.lang.SystemJ.arraycopy(bits, 0, tempBits, 0, this.actualArrayLength);
            bits = tempBits;
        }
        ...
        public bool get(int pos) {
            if (pos < 0) {
                ...
                throw new java.lang.IndexOutOfBoundsException("Negative index specified"); //$NON-NLS-1$
            }
        ...
        public BitSet get(int pos1, int pos2) {
            ...
            long factor2 = java.dotnet.lang.Operator.shiftRightUnsignet( (~0L) , (ELM_SIZE - (pos2 & RIGHT_BITS)));

            if (idx1 == idx2) {
                long result = java.dotnet.lang.Operator.shiftRightUnsignet ((bits[idx1] & (factor1 & factor2)) , (pos1 % ELM_SIZE));
                if (result == 0) {
                    return new BitSet(0);
                }
                return new BitSet(new long[] { result }, needClearJ, 1, true);
            }
            ...
            if (numBitsToShift != 0) {
                for (int i = 0; i < newbits.Length; i++) {
                    ...
                    // sign
                    newbits[i] = java.dotnet.lang.Operator.shiftRightUnsignet(newbits[i], (numBitsToShift));
            ...
            return new BitSet(newbits, needClearJ, actualLen,
        ...
        public int cardinality() {
            ...
            for (int idx = 0; idx < length; idx++) {
                ...
                count += pop(java.dotnet.lang.Operator.shiftRightUnsignet(bits[idx], 32));
            }
            return count;
        }

        private  int pop(long x) {
            x = x - (java.dotnet.lang.Operator.shiftRightUnsignet(x, 1) & 0x55555555);
            x = (x & 0x33333333) + (java.dotnet.lang.Operator.shiftRightUnsignet(x, 2) & 0x33333333);
            x = (x + java.dotnet.lang.Operator.shiftRightUnsignet(x, 4)) & 0x0f0f0f0f;
            x = x + java.dotnet.lang.Operator.shiftRightUnsignet(x, 8);
            x = x + java.dotnet.lang.Operator.shiftRightUnsignet(x, 16);
