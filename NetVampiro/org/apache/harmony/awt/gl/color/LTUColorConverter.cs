...
namespace org.apache.harmony.awt.gl.color
{
    ...
    internal class LUTColorConverter
    {
            ...
            if (from8lRGBtosRGB_LUT == null)
            {
                ...
                for (int i = 0; i < 256; i++)
                {
                    ...
                    v = (v <= 0.04045f) ? v / 12.92f :
                        (float)java.lang.Math.pow((v + 0.055) / 1.055, 2.4);
                    from8lRGBtosRGB_LUT[i] = (byte)java.lang.Math.round(v * 255.0f);
                }
            }
        ...
        public static byte[] getFrom16lRGBtosRGB_LUT()
        {
            if (from16lRGBtosRGB_LUT == null)
            {
                ...
                for (int i = 0; i < 65536; i++)
                {
                    ...
                    v = (v <= 0.04045f) ? v / 12.92f :
                        (float)java.lang.Math.pow((v + 0.055) / 1.055, 2.4);
                    from16lRGBtosRGB_LUT[i] = (byte)java.lang.Math.round(v * 255.0f);
                }
            }
            ...
            if (fromsRGBto8lRGB_LUT == null)
            {
                ...
                for (int i = 0; i < 256; i++)
                {
                    ...
                    v = (v <= 0.0031308f) ? v * 12.92f :
                        ((float)java.lang.Math.pow(v, 1.0 / 2.4)) * 1.055f - 0.055f;
                    fromsRGBto8lRGB_LUT[i] = (byte)java.lang.Math.round(v * 255.0f);
                }
            }
        ...
        public static short[] getFromsRGBto16lRGB_LUT()
        {
            if (fromsRGBto16lRGB_LUT == null)
            {
                ...
                for (int i = 0; i < 256; i++)
                {
                    ...
                    v = (v <= 0.0031308f) ? v * 12.92f :
                        ((float)java.lang.Math.pow(v, 1.0 / 2.4)) * 1.055f - 0.055f;
                    fromsRGBto16lRGB_LUT[i] = (short)java.lang.Math.round(v * 65535.0f);
                }
            }
        ...
        public static bool is_LINEAR_RGB_CS(java.awt.color.ColorSpace cs)
        {
            return (cs == LINEAR_RGB_CS);
        }

        public static bool is_LINEAR_GRAY_CS(java.awt.color.ColorSpace cs)
        {
            return (cs == LINEAR_GRAY_CS);
        }

        public static bool is_sRGB_CS(java.awt.color.ColorSpace cs)
        {
            return (cs == sRGB_CS);
        }
