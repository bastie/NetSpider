...
/*
    ...
    private readonly java.util.HashSet<PolicyEntry> grants = new java.util.HashSet<PolicyEntry>();
    ...
    private readonly java.util.Map<Object, java.util.Collection<Permission>> cache = new java.util.HashMap<Object, java.util.Collection<Permission>>();//new java.util.WeakHashMap<Object, Collection<Permission>>();

    // A specific parser for a particular policy file format.
    private readonly DefaultPolicyParser parser;
    ...
    public DefaultPolicy(bool needRefresh) :
    ...
    public DefaultPolicy(DefaultPolicyParser dpr, bool needRefresh) {
        parser = dpr;
        initialized = false;
        if (needRefresh) {
            refreshImpl(null);      	
    ...
    public PermissionCollection getPermissions(ProtectionDomain pd) {
        ...
        java.util.Collection<Permission> pc = cache.get(pd);
        if (pc == null) {
            ...
            lock (cache) {
                ...
                pc = cache.get(pd);
                if (pc == null) {
                    pc = new java.util.HashSet<Permission>();
                    java.util.Iterator<PolicyEntry> it = grants.iterator();
                    while (it.hasNext()) {
    ...
    public PermissionCollection getPermissions(CodeSource cs) {
        ...
        java.util.Collection<Permission> pc = cache.get(cs);
        if (pc == null) {
            //have to synchronize to exclude cache pollution after refresh
            lock (cache) {
     ...
     * This method is declared lock to avoid concurrent reloading.
     * 
    ...
    public void refresh(URIParameter para) {
        lock (this){
        refreshImpl(para);
        }
    }
    ...
    private void refreshImpl(URIParameter para) {
        Set<PolicyEntry> fresh = new java.util.HashSet<PolicyEntry>();
        java.util.Properties system = new java.util.Properties(AccessController
                .doPrivileged(new PolicyUtils.SystemKit()));
        system.setProperty("/", File.separator); //$NON-NLS-1$
        java.net.URL[] policyLocations = PolicyUtils.getPolicyURLs(system,
                JAVA_SECURITY_POLICY, POLICY_URL_PREFIX);
        bool needLoadSysProp = true;
        if (para != null) {			
            try {
                java.net.URL paramURL = para.getURI().toURL();
